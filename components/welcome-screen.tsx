"use client"

import { Camera } from "lucide-react"
import { Button } from "@/components/ui/button"
import { config } from "@/lib/js/config"

interface WelcomeScreenProps {
  onStart: () => void
}

export default function WelcomeScreen({ onStart }: WelcomeScreenProps) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden bg-background text-foreground">
      <div className="absolute inset-0">
        <div
          className="absolute inset-0"
          style={{
            backgroundImage:
              "radial-gradient(circle at top, rgba(255, 226, 131, 0.22), transparent 58%), radial-gradient(circle at 80% 90%, rgba(232, 90, 71, 0.2), transparent 50%), linear-gradient(160deg, rgba(12, 12, 16, 0.96), rgba(26, 26, 34, 0.98))",
          }}
        />
        <div
          className="absolute inset-0 opacity-30"
          style={{
            backgroundImage:
              "linear-gradient(90deg, rgba(255, 255, 255, 0.08) 1px, transparent 1px), linear-gradient(rgba(255, 255, 255, 0.08) 1px, transparent 1px)",
            backgroundSize: "56px 56px",
          }}
        />
      </div>

      <div className="max-w-2xl mx-auto text-center relative z-10">
        <div className="mb-10">
          <div className="inline-flex items-center justify-center rounded-2xl border border-border/60 bg-card/70 px-6 py-5 shadow-[0_0_40px_rgba(80,100,255,0.15)]">
            <span className="text-sm md:text-base tracking-[0.3em] uppercase text-muted-foreground">
              WSGVR Affiliate Mixer
            </span>
          </div>
          <h1 className="mt-8 text-5xl md:text-6xl font-semibold tracking-tight text-balance">
            {config.branding.primaryText}
          </h1>
          <p className="mt-3 text-base md:text-lg text-muted-foreground">{config.branding.secondaryText}</p>
          <p className="mt-4 text-sm md:text-base text-muted-foreground">
            Thursday, January 29, 2026 • 3:00 PM – 6:00 PM (PST)
          </p>
          <p className="text-sm md:text-base text-muted-foreground">
            Holiday Inn Monterey Park • 400 N. Atlantic Blvd., Monterey Park, CA
          </p>
        </div>

        <div className="mb-10 space-y-5">
          <p className="text-lg text-muted-foreground leading-relaxed">
            Raise a glass, grab a photo, and celebrate Happy New Year with the WSGVR affiliate community.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-muted-foreground">
            <div className="flex items-center gap-2">
              <Camera className="h-4 w-4" />
              <span>Instant capture</span>
            </div>
            <div className="h-4 w-px bg-border/60" />
            <span>Happy Hour frame</span>
            <div className="h-4 w-px bg-border/60" />
            <span>Share-ready</span>
          </div>
        </div>

        <Button
          size="lg"
          onClick={onStart}
          className="text-base md:text-lg px-12 py-6 h-auto rounded-full shadow-[0_12px_40px_rgba(90,110,255,0.25)] hover:shadow-[0_16px_48px_rgba(90,110,255,0.35)] transition-all"
        >
          <Camera className="mr-2 h-5 w-5" />
          Start Photobooth
        </Button>

        <p className="mt-12 text-xs text-muted-foreground">{config.branding.footerText}</p>
      </div>
    </div>
  )
}
